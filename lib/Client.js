...
class Client extends EventEmitter {
    constructor(options) {
        ...
        agent.on('online', () => {
            console.log(`lt:Client[${this.id}]: client online ${id}`);
            clearTimeout(this.graceTimeout);
        });

        agent.on('offline', () => {
            console.log(`lt:Client[${this.id}] client offline ${id}`);
    ...
    handleRequest(req, res) {
        console.log(`lt:Client[${this.id}] > ${req.url}`);
        const opt = {
            path: req.url,
            agent: this.agent,
            method: req.method,
    ...
    handleUpgrade(req, socket) {
        console.log(`lt:Client[${this.id}] > [up] ${req.url}`);
