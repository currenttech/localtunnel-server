...
class Client extends EventEmitter {
    constructor(options) {
        ...
        this.graceTimeout = setTimeout(() => {
            // console.log('#1 graceTimeout caller');
            this.close();
        }, 300000).unref();

        agent.on('online', () => {
            // console.log(`Client[${this.id}]: client online ${id}`);
            clearTimeout(this.graceTimeout);
        });

        agent.on('offline', () => {
            // console.log(`Client[${this.id}] client offline ${id}`);
            ...
            this.graceTimeout = setTimeout(() => {
                // console.log('#2 graceTimeout caller');
                this.close();
            }, 1000).unref();
        });
    ...
    handleRequest(req, res) {
        // console.log(`Client[${this.id}] > ${req.url}`);
        const opt = {
            path: req.url,
            agent: this.agent,
            method: req.method,
        ...
        clientReq.once('error', (err) => {
            ...
            // console.log(`clientReg error ${err}`);
        });
    ...
    handleUpgrade(req, socket) {
        // console.log(`Client[${this.id}] > [up] ${req.url}`);
