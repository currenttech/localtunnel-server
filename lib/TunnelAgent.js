...
class TunnelAgent extends Agent {
    constructor(options = {}) {
        ...
        this.optionsClientId = options.clientId;
    ...
    listen() {
        ...
        server.on('error', (err) => {
            // console.log('hit client error', err);
        ...
        return new Promise((resolve) => {
            server.listen(() => {
                ...
                // console.log(`TunnelAgent[${this.optionsClientId}]: tcp server listening on port: ${port}`);
    ...
    _onClose() {
        this.closed = true;
        // console.log(`TunnelAgent[${this.optionsClientId}]: closed tcp socket`);
    ...
    _onConnection(socket) {
        ...
        if (this.connectedSockets >= this.maxTcpSockets) {
            // console.log(`TunnelAgent[${this.optionsClientId}]: no more sockets allowed`);
            socket.destroy();
            return false;
        }

        socket.once('close', (hadError) => {
            // console.log(`TunnelAgent[${this.optionsClientId}]: closed socket (error: ${hadError})`);
            ...
            // console.log(`TunnelAgent[${this.optionsClientId}]: connected sockets: ${this.connectedSockets}`);
            if (this.connectedSockets <= 0) {
                // console.log(`TunnelAgent[${this.optionsClientId}]: all sockets disconnected`);
                this.emit('offline');
            }
        });
        ...
        socket.once('error', (err) => {
            ...
            // console.log('socket error in agent', err);
            socket.destroy();
        });
        // console.log('**before count check**');
        if (this.connectedSockets === 0) {
            this.emit('online');
        }

        this.connectedSockets += 1;
        // console.log(`TunnelAgent[${this.optionsClientId}]: new connection from: ${socket.address().address}:${socket.address().port}`);

        // if there are queued callbacks, give this socket now and don't queue into available
        const fn = this.waitingCreateConn.shift();
        if (fn) {
            // console.log(`TunnelAgent[${this.optionsClientId}]: giving socket to queued conn request`);
            setTimeout(() => {
                fn(null, socket);
            }, 0);
            return;
    ...
    createConnection(options, cb) {
        ...
        // console.log(`TunnelAgent[${this.optionsClientId}]: create connection`);
        ...
        if (!sock) {
            ...
            // console.log(`TunnelAgent[${this.optionsClientId}]: waiting connected: ${this.connectedSockets}`);
            // console.log(`TunnelAgent[${this.optionsClientId}]: waiting available: ${this.availableSockets.length}`);
            return;
        }

        // console.log(`TunnelAgent[${this.optionsClientId}]: socket given`);
        cb(null, sock);
    }
