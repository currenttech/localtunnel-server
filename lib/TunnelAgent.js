...
class TunnelAgent extends Agent {
    ...
    listen() {
        ...
        return new Promise((resolve) => {
            server.listen(() => {
                ...
                console.log(`lt:TunnelAgent[${this.optionsClientId}]: tcp server listening on port: ${port}`);
    ...
    _onClose() {
        this.closed = true;
        console.log(`lt:TunnelAgent[${this.optionsClientId}]: closed tcp socket`);
    ...
    _onConnection(socket) {
        ...
        if (this.connectedSockets >= this.maxTcpSockets) {
            console.log(`lt:TunnelAgent[${this.optionsClientId}]: no more sockets allowed`);
            socket.destroy();
            return false;
        }

        socket.once('close', (hadError) => {
            console.log(`lt:TunnelAgent[${this.optionsClientId}]: closed socket (error: ${hadError})`);
            ...
            console.log(`lt:TunnelAgent[${this.optionsClientId}]: connected sockets: ${this.connectedSockets}`);
            if (this.connectedSockets <= 0) {
                console.log(`lt:TunnelAgent[${this.optionsClientId}]: all sockets disconnected`);
                this.emit('offline');
            }
        });
        ...
        this.connectedSockets += 1;
        console.log(`lt:TunnelAgent[${this.optionsClientId}]: new connection from: ${socket.address().address}:${socket.address().port}`);

        // if there are queued callbacks, give this socket now and don't queue into available
        const fn = this.waitingCreateConn.shift();
        if (fn) {
            console.log(`lt:TunnelAgent[${this.optionsClientId}]: giving socket to queued conn request`);
            setTimeout(() => {
                fn(null, socket);
            }, 0);
            return;
    ...
    createConnection(options, cb) {
        ...
        console.log(`lt:TunnelAgent[${this.optionsClientId}]: create connection`);
        ...
        if (!sock) {
            ...
            console.log(`lt:TunnelAgent[${this.optionsClientId}]: waiting connected: ${this.connectedSockets}`);
            console.log(`lt:TunnelAgent[${this.optionsClientId}]: waiting available: ${this.availableSockets.length}`);
            return;
        }

        console.log(`lt:TunnelAgent[${this.optionsClientId}]: socket given`);
        cb(null, sock);
    }
